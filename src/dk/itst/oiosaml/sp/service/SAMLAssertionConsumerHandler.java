...
//		}
    ...
    private void concatPidOrPhoneOnUrlByUidFromOneDB(StringBuilder url, String uid, RequestContext ctx) throws UnsupportedEncodingException {
    	...
    	if (isNeedPid) {
    		...
    		if (isNeedPhone) {
    			sql.append(", ")
    				.append(conf.getString("oiosaml-sp.login.phone.column"));
    		}
    	} else {
    		sql.append(conf.getString("oiosaml-sp.login.phone.column"));
    	}
    	sql.append(" FROM ")
	    	.append(conf.getString("oiosaml-sp.login.table.all"))
	    	.append(" WHERE ")
	    	.append(conf.getString("oiosaml-sp.login.uid.column"))
	    	.append(" = ?");
    ...
    private void concatPidOrPhoneOnUrlByUidFromTwoDB(StringBuilder url, String uid, RequestContext ctx) throws UnsupportedEncodingException {
    	...
    	if (isNeedPhone) {
    		...
    		sql.append(conf.getString("oiosaml-sp.login.pid.column"))
	    		.append(" FROM ")
	    		.append(conf.getString("oiosaml-sp.login.table.first"))
	    		.append(" WHERE ")
	    		.append(conf.getString("oiosaml-sp.login.uid.column"))
	    		.append(" = ?");
        	String pid = null;
        	...
        	if (StringUtils.isNotBlank(pid)) {
        		...
        		sql.append("SELECT ")
        			.append(conf.getString("oiosaml-sp.login.phone.column"))
        			.append(" FROM ")
        			.append(conf.getString("oiosaml-sp.login.table.second"))
        			.append(" WHERE ")
        			.append(conf.getString("oiosaml-sp.login.pid.column2"))
        			.append(" = ?");
        		try {
